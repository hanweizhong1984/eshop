...
function GoodsCommentsHtml(page)
{
    ...
    $.ajax({
        ...
        success:function(result)
        {
            ...
            {
                $('.goods-page-no-data').removeClass('none');
                $('.goods-page-no-data span').text($.cookie('think_var')=='en-us'?'There is no comment data':'没有评论数据');
            }
        },
        ...
        if(__user_id__ != 0)
        {
            ...
            $.ajax({
                ...
                success: function(result)
                {
                    ...
                    if(result.code == 0)
                    {
                        var ft = result.data.text;
                        if ($.cookie('think_var') == 'en-us') {
                            ft = ft == '收藏' ? 'My Favorite' : 'Collected';
                        }
                        $this.find('.goods-favor-text').text(ft);
