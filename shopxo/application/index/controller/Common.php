...
class Common extends Controller
{
    ...
    private function countDown()
    {
        ...
        //$start = $config['common_sale_start_time']['value'];
        $start = MyC('common_sale_start_time');
        $now = date('Y-m-d H:i:s', time());
        if ($start >= $now) {
            $second = strtotime($start) - strtotime('now');
            $this->assign('second', $second);
            echo $this->fetch('index/countdown');
            exit();
        }
    }
    /**
    ...
    private function emptyCart(){
        ...
        $CartResult = Db::name('Cart')->where($where)->select();
        foreach($CartResult as &$v) {
            $add_time = $v['add_time'];
            $add_timeYmd = date("Y-m-d H:i:s",$add_time);
            $now = date('Y-m-d H:i:s', time());
            $second = strtotime('now')-strtotime($add_timeYmd);
            $minute = intval($second/60);
            //如果购物车中有超过10天的商品,就清空超时的商品
            if($minute > 14400){
                // 删除
                $where = [
                    'id'        => $v['id'],
    ...
    private function SiteStstusCheck()
    {
        if(MyC('home_site_state') != 1)
        {
            ...
            if(IS_AJAX)
            {
                ...
                exit('<div style="text-align: center;margin-top: 15%;font-size: 18px;color: #ff0000;">' .MyC('home_site_close_reason', '网站维护中...', true).'</div>');
            }
        }
    }
}
