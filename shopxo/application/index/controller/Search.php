...
class Search extends Common
{
    ...
    public function GoodsList()
    {
        ...
        $check = $this->params;
        $orderfield = array('default','sales_count','min_price');
        $ordertype = array('desc','asc');

        // 分类id 数字8位 默认0
        if(isset($check['category_id']))
        {
            if(!preg_match("/^\d{1,8}$/",$check['category_id']))
            {
                $check['category_id']=0;
            }
        }
        ...
        if(isset($check['page']))
        {
            if(!preg_match("/^\d{1,3}$/",$check['page']))
            {
                $check['page']=1;
            }
        }
        // order_by_field:排序字段 只有三种  default、sales_count、min_price
        if(!in_array($check['order_by_field'],$orderfield)){
            $check['order_by_field']='default';
        }

        // order_by_type:排序只有2种 desc  asc
        if(!in_array($check['order_by_type'],$ordertype)){
            $check['order_by_type']='asc';
        }

        $ret = SearchService::GoodsList($check);

        // 搜索记录
        SearchService::SearchAdd($check);

        // 无数据直接返回
        if(empty($ret['data']['data']) || $ret['code'] != 0)
        {
            return DataReturn(lang('没有更多数据啦'), -100);
        }
