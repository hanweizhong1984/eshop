...
class AdminService
{
    ...
    public static function AdminSave($params = [])
    {
        ...
        // 管理员自己密码到期后未输入密码会提醒输入新密码
        if(isset($params['id']) && $params['id'] == session('admin')["id"]
                 && time() > session('admin')["out_time"]  && empty($params['login_pwd'])){
            return DataReturn('密码'.config('shopxo.out_day').'天已到期，请输入新的登录密码', -1);
        }
    ...
    public static function AdminInsert($params = [])
    {
        ...
        $data = [
            ...
            'add_time'      => time(),
            'out_time'      => time() + config('shopxo.out_day')*24*60*60,
        ];
    ...
    public static function Login($params = [])
    {
        ...
        // 获取管理员
        $admin = Db::name('Admin')->field('id,username,login_pwd,login_salt,mobile,login_total,role_id,out_time')->where(['username'=>$params['username']])->find();
        if(empty($admin))
        {
