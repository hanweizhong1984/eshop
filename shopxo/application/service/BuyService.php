...
class BuyService
{
    ...
    public static function CartSave($params = [])
    {
        ...
        if(empty($temp))
        {
            ...
            {
                return DataReturn(lang('加入成功'), 0, self::UserCartTotal($params));
            }
        } else {
            $data['upd_time'] = time();
        ...
        return DataReturn(lang('加入失败'), -100);
    }

    public static function Buynow($params = [])
    {
        $ret=self::fenleixiangou($params);

        if($ret['code']!=0){
            return DataReturn($ret['msg'],$ret['code']);
        }else {
            return DataReturn('', 0);
        }
    }

    public static function fenleixiangou($params = []){
        // 是否分类限购
        $common_limit_buy = MyC('common_limit_buy', 0);
        if($common_limit_buy == 1){
            ...
            $field='goods_id';
            $CartResult = Db::name('Cart')->where($where)->field($field)->select();
            //循环购物车中的商品
            foreach($CartResult as &$v)
            {
                //先查购物车中goods的category
                $where=[];
                $where['goods_id'] = $v['goods_id'];
                $field='category_id';
                $result = Db::name('GoodsCategoryJoin')->where($where)->field($field)->find();
            ...
            if($FT>1){
                return DataReturn($cookie=='en-us'?'The goods under FT category have been added to the shopping cart. Please select the goods under other categories!':'购物车中已经加入了FT分类下的商品了，请选择加入其他分类的商品！', -1);
            }else if($AP>1){
                return DataReturn($cookie=='en-us'?'The goods under AP category have been added to the shopping cart. Please select the goods under other categories!':'购物车中已经加入了AP分类下的商品了，请选择加入其他分类的商品！', -1);
            }else if($AC>1){
                return DataReturn($cookie=='en-us'?'The goods under AC category have been added to the shopping cart. Please select the goods under other categories!':'购物车中已经加入了AC分类下的商品了，请选择加入其他分类的商品！', -1);
            }

            $where=[];
            //order里查用户已支付的订单明细
            $where['user_id'] = $userId;
            $where['status'] = [1,2,3];//只判断只在进行中的订单的商品分类是否已经存在
            $where['pay_status'] = [0,1];
            $field='id';
            $Orders = Db::name('Order')->where($where)->field($field)->select();
            foreach($Orders as &$order){
                $orderId= $order['id'];
                $where =[];
                $where['order_id'] = $orderId;
                $field='goods_id';
                $OrdeDetail = Db::name('OrderDetail')->where($where)->field($field)->find();
                $goodsId = $OrdeDetail['goods_id'];

//                $where =[];
//                $where['id'] = $goodsId;
//                $field='is_shelves';
//                $Goods = Db::name('Goods')->where($where)->field($field)->find();
//                $is_shelves =$Goods['is_shelves'];

                $where =[];
                $where['goods_id'] = $goodsId;
                $field='category_id';
                $GoodsCategory = Db::name('GoodsCategoryJoin')->where($where)->field($field)->find();
                $categoryId=$GoodsCategory['category_id'];
                //1(FT),2(AP),3(AC)
                if($categoryId==1 && $FT>0){
                    return DataReturn($cookie=='en-us'?'You have purchased goods under FT category. Please select goods under other categories!':'您已购买过FT分类下的商品了，请选择其他分类的商品！', -1);
                }else if($categoryId==2 &&$AP >0){
                    return DataReturn($cookie=='en-us'?'You have purchased goods under AP category. Please select goods under other categories!':'您已购买过AP分类下的商品了，请选择其他分类的商品！', -1);
                }else if($categoryId==3 &&$AC >0){
                    return DataReturn($cookie=='en-us'?'You have purchased goods under AC category. Please select goods under other categories!':'您已购买过AC分类下的商品了，请选择其他分类的商品！', -1);
                }
            }
        }
    }
    ...
    public static function CartDelete($params = [])
    {
        ...
        {
            return DataReturn(lang('删除成功'), 0, self::UserCartTotal($params));
        }
    ...
    public static function CartStock($params = [])
    {
        ...
        if(empty($cart))
        {
            return DataReturn(lang('请先加入购物车'), -1);
        }
        $cart[0]['stock'] = $stock;
        ...
        if(Db::name('Cart')->where($where)->update($data))
        {
            return DataReturn(lang('更新成功'), 0);
        }
        return DataReturn(lang('更新失败'), -100);
    }
...
//            ],
        ...
        // 用户地址id纯数字8位以内校验(提交订单页面选择其他地址)
        if(isset($params['address_id']))
        {
            if(!preg_match("/^\d{1,8}$/",$params['address_id']))
            {
                return DataReturn('购物车id有误', -1);
            }
        }
        // 支付分类id纯数字1-9校验(提交订单页面选择其他支付方式)
        if(isset($params['payment_id']))
        {
            if(!preg_match("/^[1-9]$/",$params['payment_id']))
            {
                return DataReturn('购物车id有误', -1);
            }
        }
    ...
    public static function BuyGoodsCheck($params = [])
    {
        ...
        foreach($params['goods'] as $v)
        {
            ...
            {
                return DataReturn(lang('商品已下架[').$goods['title'].']', -1);
            }
            if($v['stock'] > $goods['inventory'])
            {
                return DataReturn(lang('购买数量超过商品库存数量[').$goods['title'].']['.$v['stock'].'>'.$goods['inventory'].']', -1);
            }
            if($goods['buy_min_number'] > 1 && $v['stock'] < $goods['buy_min_number'])
            {
                return DataReturn(lang('低于商品起购数量[').$goods['title'].']['.$v['stock'].'<'.$goods['buy_min_number'].']', -1);
            }
            if($goods['buy_max_number'] > 0 && $v['stock'] > $goods['buy_max_number'])
            {
                return DataReturn(lang('超过商品限购数量[').$goods['title'].']['.$v['stock'].'>'.$goods['buy_max_number'].']', -1);
            }
        }
     ...
     * @param   [array]          $params [输入参数]
     */
    public static function BuyLimitCheck($params = [])
    {
        $day = MyC('common_limit_day');
        $limit_total_price = MyC('common_limit_total_price');
        if (empty($day) || empty($limit_total_price)) {
            return DataReturn(lang('服务数据不存在'), -1);
        }
        //系统设定几天前的时间戳（比如30天前）
        $start = strtotime("-" . $day . " day");
        $end = time();

        // 获取该用户在指定时间区间内状态为1已确认/待支付, 2已支付/待发货, 3已发货/待收货, 4已完成那些订单的总金额
        $sum = Db::name('Order')->where(['user_id' => $params['user'], 'cancel_time' => 0, 'user_is_delete_time' => 0, 'status' => [1, 2, 3, 4]])->whereTime('add_time', 'between', [$start, $end])->sum('total_price');
        if ($sum + $params['base']['total_price'] > $limit_total_price) {
            $ch_msg = '在' . $day . '天内您已累计消费超过限额' . $limit_total_price . '元,过往总消费了' . $sum . '元.';
            $eng_msg = 'You have exceeded the limit of ' . $limit_total_price . ' yuan in ' . $day . ' days, and the total consumption in the past has been ' . $sum . ' yuan. ';
            return DataReturn(cookie('think_var') == 'en-us' ? $eng_msg : $ch_msg, -1);
        }
    ...
    public static function OrderInsert($params = [])
    {
        ...
        //限额检查
        $check = self::BuyLimitCheck(['user'=>$params['user']['id'],'base'=>$buy['data']['base']]);
        if($check['code'] != 0)
        {
            return $check;
        }
        ...
        $order = [
            ...
            'order_no'              => substr(date('Ymd'),-5).GetNumberCode(5),
            'user_id'               => $params['user']['id'],
        ...
        switch($order['status'])
        {
            ...
            case 1 :
                $msg = lang('提交成功');
                //$msg =TranslationService::language('提交成功');
    ...
    public static function OrderPayBeginCheck($params = [])
    {
        ...
        if(!empty($order_detail))
        {
            foreach($order_detail as $v)
            {
                ...
                // 库存
                if(isset($goods['is_deduction_inventory']) && $goods['is_deduction_inventory'] == 2)
                {
                    // 先判断商品库存是否不足
