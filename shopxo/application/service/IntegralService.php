...
class IntegralService
{
     ...
     */
    public static function UserIntegralLogAdd($user_id, $original_integral, $new_integral, $msg = '',$msg_en='', $type = 0, $operation_id = 0)
    {
        $data = array(
        ...
        if(Db::name('UserIntegralLog')->insertGetId($data) > 0)
        {
            ...
            $detail_en = $msg_en.lang('积分').$type_msg.$integral;
            MessageService::MessageAdd($user_id, '积分变动', $detail,$detail_en);
        ...
        if(!empty($goods_all))
        {
            foreach($goods_all as $goods_id)
            {
                ...
                if(!empty($give_integral))
                {
                    ...
                    $msg = '订单商品完成赠送';
                    $msg_en = lang('订单商品完成赠送');
                    // 积分日志
                    self::UserIntegralLogAdd($user['id'], $user_integral, $user_integral + $give_integral, $msg, $msg_en, 1);
                }
            }
