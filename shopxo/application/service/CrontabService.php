        ...
        if(!empty($order))
        {
            ...
            foreach($order as $v)
            {
                ...
                if(Db::name('Order')->where(['id'=>$v['id'], 'status'=>1])->update($upd_data))
                {
                    ...
                    {
                        $detail = '订单超时关闭';
                        $detail_en = lang('订单超时关闭');
                        // 用户消息
                        MessageService::MessageAdd($v['user_id'], '订单关闭', $detail, $detail_en, 1, $v['id']);
                ...
                if(Db::name('Order')->where(['id'=>$v['id'], 'status'=>3])->update($upd_data))
                {
                    ...
                    if($ret['code'] == 0)
                    {
                        ...
                        if($ret['code'] == 0)
                        {
                            // 消息通知
                            $detail = '订单自动收货成功';
                            $detail_en = 'Order received automatically';
                            // 用户消息
                            MessageService::MessageAdd($v['user_id'], '订单收货', $detail, $detail_en, 1, $v['id']);
