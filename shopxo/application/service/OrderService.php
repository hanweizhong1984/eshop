...
class OrderService
{
    ...
    public static function Respond($params = [])
    {
        ...
        if(isset($ret['code']) && $ret['code'] == 0)
        {
            ...
            {
                return DataReturn($cookie=='en-us'?'Submitted successfully, to be confirmed by administrator':'提交成功、待管理员确认', 0);
            }
        }
        return $ret;
    }
    ...
    public static function OrderPayHandle($params = [])
    {
        ...
        $detail_en = 'The order payment is successful, the amount is ' . PriceBeautify($params['order']['total_price']) . ' yuan';
        MessageService::MessageAdd($params['order']['user_id'], '订单支付', $detail, $detail_en, 1, $params['order']['id']);
    ...
    public static function OrderListWhere($params = [])
    {
        ...
        {
            $where[] = ['order_no', 'in', $params['ordernos']];
        }
...
//            3 => array('id' => 3, 'name' => '部分退款'),
            ...
            foreach($data as &$v)
            {
                ...
                if(isset($v['user_id']))
                {
                    if(isset($params['is_public']) && $params['is_public'] == 0)
                    {
                        ...
                        if(strstr($v['user']['username'],'@')){
                            $v['user']['username']=substr($v['user']['username'],0,strpos($v['user']['username'], '@'));
                        }
                    }
                }
    ...
    public static function OrderCancel($params = [])
    {
        ...
        if(Db::name('Order')->where($where)->update($upd_data))
        {
            ...
            // 用户消息
            MessageService::MessageAdd($order['user_id'], '订单取消', '订单取消成功','Order cancelled succeeded', 1, $order['id']);
            ...
            // 提交事务
            Db::commit();
            return DataReturn($cookie=='en-us'?'Cancelled successfully':'取消成功', 0);
        }
    ...
    public static function OrderCollect($params = [])
    {
        ...
        if(Db::name('Order')->where($where)->update($upd_data))
        {
            ...
            // 用户消息
            MessageService::MessageAdd($order['user_id'], '订单收货', '订单收货成功', 'Order received succeeded', 1, $order['id']);
            ...
            // 提交事务
            Db::commit();
            return DataReturn(lang('收货成功'), 0);
        }

        // 事务回滚
        Db::rollback();
        return DataReturn(lang('收货失败'), -1);
    }
    ...
    public static function OrderConfirm($params = [])
    {
        ...
        if(Db::name('Order')->where($where)->update($upd_data))
        {
            ...
            // 用户消息
            MessageService::MessageAdd($order['user_id'], '订单确认', '订单确认成功', 'Order confirmed succeeded', 1, $order['id']);
    ...
    public static function OrderDelete($params = [])
    {
        ...
        {
            // 用户消息
            MessageService::MessageAdd($order['user_id'], '订单删除', '订单删除成功', 'Order deleted succeeded', 1, $order['id']);

            return DataReturn(lang('删除成功'), 0);
        }
        return DataReturn(lang('删除失败或资源不存在'), -1);
    }
    ...
    public static function OrderStatusStepTotal($params = [])
    {
        ...
        $order_status_list = $common_order_user_status;
