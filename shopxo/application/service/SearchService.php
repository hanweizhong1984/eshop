...
class SearchService
{
    ...
    public static function GoodsList($params = [])
    {
        ...
        {
            $key = str_replace('&amp;amp;','&',$params['keywords']);
            $where[] = ['g.title|g.model|g.simple_desc|g.seo_title|g.seo_keywords|g.seo_keywords|gc.name', 'like', '%'.$key.'%'];
        }
        ...
        if($result['total'] > 0)
        {
            ...
            $n = 20;
            $m = intval(($page-1)*$n);
            $goods = GoodsService::CategoryGoodsList(['where'=>$where, 'm'=>$m, 'n'=>$n, 'order_by'=>$order_by,'is_spec'=>true]);
