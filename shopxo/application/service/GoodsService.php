...
class GoodsService
{
    ...
    public static function ActivityList($params = [])
    {
        ...
        $data = Db::name('GoodsActivity')->where($where)->select();
    ...
    public static function ActivitySave($params = [])
    {
        ...
        {
            $data['add_time'] = time();
            if(Db::name('GoodsActivity')->insertGetId($data) > 0)
            {
            ...
            $data['upd_time'] = time();
            if(Db::name('GoodsActivity')->where(['id'=>intval($params['id'])])->update($data))
            {
    ...
    public static function ActivityDelete($params = [])
    {
        ...
        // 删除操作
        if(Db::name('GoodsActivity')->where(['id'=>$params['id']])->delete())
        {
        ...
        if (!empty($data) && is_array($data)) {
            ...
            foreach ($data as &$v) {
                if (!empty($v['goods_ids']) && is_array($v['goods_ids'])) {
                    //$res = self::GoodsList(['where'=>['id'=>$v['goods_ids'], 'is_home_recommended'=>1, 'is_shelves'=>1], 'm'=>0, 'n'=>10000, 'field'=>'*','is_spec'=>true]);
                    $res = self::GoodsList(['where' => ['is_home_recommended' => 1, 'is_shelves' => 1, 'id' => $v['goods_ids']], 'm' => 0, 'n' => 12, 'field' => '*', 'is_spec' => true]);
                    $v['goods'] = $res['data'];
                }
            }
        }
    ...
    {
        return (int)Db::name('Goods')->alias('g')->join(['__GOODS_CATEGORY_JOIN__' => 'gci'], 'g.id=gci.goods_id')->join(['__GOODS_CATEGORY__' => 'gc'], 'gc.id=gci.category_id')->where($where)->count('DISTINCT g.id');
    }
    ...
    public static function CategoryGoodsList($params = [])
    {
        ...
        $data = Db::name('Goods')->alias('g')->join(['__GOODS_CATEGORY_JOIN__' => 'gci'], 'g.id=gci.goods_id')->join(['__GOODS_CATEGORY__' => 'gc'], 'gc.id=gci.category_id')->field($field)->where($where)->group('g.id')->order($order_by)->limit($m, $n)->select();
    ...
    public static function GoodsDataHandle($params, $data)
    {
        ...
        return DataReturn(lang('处理成功'), 0, $data);
    }
    ...
    public static function GoodsFavor($params = [])
    {
        ...
        if (empty($temp)) {
            ...
            if (Db::name('GoodsFavor')->insertGetId($data) > 0) {
                return DataReturn(lang('收藏成功'), 0, [
                    'text' => '已收藏',
                    'status' => 1,
                    'count' => self::GoodsFavorTotal(['goods_id' => $data['goods_id']]),
                ]);
            } else {
                return DataReturn(lang('收藏失败'));
            }
        } else {
            // 是否强制收藏
            ...
            if (Db::name('GoodsFavor')->where($data)->delete() > 0) {
                return DataReturn(lang('取消成功'), 0, [
                    'text' => '收藏',
                    'status' => 0,
                    'count' => self::GoodsFavorTotal(['goods_id' => $data['goods_id']]),
                ]);
            } else {
                return DataReturn(lang('取消失败'));
            }
        }
    }
    ...
    public static function IsUserGoodsFavor($params = [])
    {
        ...
        $temp = Db::name('GoodsFavor')->where($data)->find();
        return DataReturn(lang('操作成功'), 0, empty($temp) ? 0 : 1);
    }
    ...
    public static function GoodsBrowseSave($params = [])
    {
        ...
        if ($status) {
            return DataReturn(lang('处理成功'), 0);
        }
        return DataReturn(lang('处理失败'), -100);
    }
    ...
    public static function GoodsSave($params = [])
    {
        ...
        if (isset($goods_id) && $goods_id > 0) {
            ...
            // 提交事务
            Db::commit();
            return DataReturn(lang('操作成功'), 0);
        }

        // 回滚事务
        Db::rollback();
        return DataReturn(lang('操作失败'), -100);
    }
    ...
    private static function GoodsSaveBaseUpdate($params, $goods_id)
    {
        ...
        if (empty($data)) {
            return DataReturn(lang('没找到商品基础信息'), -1);
        }
    ...
    public static function GoodsDelete($params = [])
    {
        ...
        Db::rollback();
        return DataReturn(lang('删除失败'), -100);
    }
     ...
     * @desc    description
     */
    public static function GoodsCategoryIds($params = [])
    {
        ...
        for ($i = 0; $i < sizeof($array); $i++) {
            $data = Db::name('GoodsCategory')->where(['name' => $array[$i], 'pid' => $pid])->find();
            if (!empty($data)) {
                $arr[$i] = $data['id'];
                $pid = $data['id'];
            }
