...
class MessageService
{
     ...
     * @param    [string]           $detail_en      [内容_英文（多語言用）]
     * @param    [int]              $business_type  [业务类型（0默认, 1订单, 2充值, 3提现, ...）]
     * @param    [int]              $business_id    [业务id]
     * @param    [int]              $type           [类型（默认0  普通消息）]
     * @return   [boolean]                          [成功true, 失败false]
     */
    public static function MessageAdd($user_id, $title, $detail,$detail_en, $business_type = 0, $business_id = 0, $type = 0)
    {
        $data = array(
            'title'             => $title,
            'detail'            => $detail,
    ...
    public static function MessageList($params = [])
    {
        ...
        if(!empty($data))
        {
            ...
            foreach($data as &$v)
            {
                ...
                //详情
                $v['detail'] = cookie('think_var')=='en-us'?$v['detail_en']:$v['detail'];
