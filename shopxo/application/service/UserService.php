...
class UserService
{
    ...
    public static function UserSave($params = [])
    {
        ...
        $data = [
            ...
            'integral'              => isset($params['integral']) ? intval($params['integral']) : 0,
    ...
    public static function UserAddressSave($params = [])
    {
        ...
        if(empty($temp))
        {
            ...
            {
                Db::commit();
                return DataReturn(lang('新增成功'), 0);
            } else {
                Db::rollback();
                return DataReturn(lang('新增失败'));
            }
        } else {
            $data['upd_time'] = time();
    ...
    public static function UserAddressDefault($params = [])
    {
        ...
        if($all_status !== false && $my_status)
        {
            // 提交事务
            Db::commit();
            return DataReturn(lang('设置成功'), 0);
        } else {
            // 回滚事务
            Db::rollback();
            return DataReturn(lang('设置失败'), -100);
        }
    }
    ...
    private static function UserHandle($user)
    {
        ...
        if(isset($user['gender']))
        {
            ...
            $user['gender_text']    =   $common_gender_list[$user['gender']]['name'];
        }
    ...
    public static function Login($params = [])
    {
        ...
        {
            return DataReturn(lang('登录账号有误'), -1);
        }
...
//        }
        ...
        //TODO:ONLY FOR TEST
        $username = $params['accounts'];
        $passwd = trim($params['pwd']);
        if($username !== 'wsadmin' || $passwd !=='Password01!'){
            // LDAP认证
            if(self::LdapAuthentication($params) === false)
            {
                return DataReturn(lang('授权失败，请联系管理员'), -3);
            }
        }
        ...
        if(empty($user))
        {
            return DataReturn(lang('帐号不存在'), -3);
        }
...
//        {
//            // 用户数据
//            $rsalt = GetNumberCode(6);
//
//            $rdata = [
//                'upd_time'      => time(),
//                'salt'          => $rsalt,
//                'pwd'           => LoginPwdEncryption($params['pwd'], $rsalt),
//                'status'        => 0,
//            ];
//
//            $rdata['email'] = $params['accounts'];
//            $rdata['username'] = $params['accounts'];
//
//            // 数据添加
//            $user_ret = self::UserInsert($rdata, $params);
//            if($user_ret['code'] == 0)
//            {
//                $user = Db::name('User')->field('id,pwd,salt,status')->where($where)->find();
//                if(empty($user))
...
//        }
        ...
        return DataReturn(lang('登录失效，请重新登录'), -100);
    }
     ...
     * @param   [array]          $params [输入参数]
     */
    public static function LdapAuthentication ($params = [], &$samname = null)
    {
        //读取ldap服务相关信息
        $filename = config('shopxo.ldap_config_dir');
        $contents = self::GetTxtcontent($filename);

        $ldapSrv = str_replace(array("\r\n", "\r", "\n"), "", substr($contents[0],8));
        $dn = str_replace(array("\r\n", "\r", "\n"), "", substr($contents[1],3));
        $username = $params['accounts'];
        $passwd = trim($params['pwd']);

        $srvuser = 'danile.pan@nike.com';
        $srvupwd = 'China098!@';
...
//        if ($ds) {
//            $r = @ldap_bind($ds, $username, $passwd);
//            if ($r) {
//                // 验证通过
//                $vr = true;
//            }
...
//        }
        ...
        if (ldap_bind($ldapConn, $srvuser, $srvupwd)) {
            $arrdn = array('dn', 'samaccountname');
            $result = ldap_search($ldapConn, $dn, "(mail=$username)", $arrdn);
            $entries = ldap_get_entries($ldapConn, $result);
            if ($entries['count'] > 0) {
                if (ldap_bind($ldapConn, $entries[0]['dn'], $passwd)) {
                    $samname = $entries[0]['samaccountname'][0];
                    $vr = true;
                }
            }
        }
     ...
     * @date    2020-08-28
     * @desc    description
     * @param   [string]          $txtfile [文件名]
     */
    public static function GetTxtcontent($txtfile)
    {
    ...
    public static function UserLoginHandle($user_id, $params = [])
    {
        ...
        if(self::UserLoginRecord($user_id))
        {
            ...
            return DataReturn($cookie=='en-us'?'Login successful':'登录成功', 0, $result);
        }
        return DataReturn($cookie=='en-us'?'Login failed. Please login again':'登录失效，请重新登录', -100);
    }
    ...
    public static function Reg($params = [])
    {
        ...
        $data = [
            'upd_time' => time(),
            'salt' => $salt,
            'pwd' => LoginPwdEncryption('Converse098!@', $salt),
        ...
        if (isset($obj) && is_object($obj)) {
            // 是否已过期
            if (!$obj->CheckExpire()) {
                return DataReturn(lang('验证码已过期'), -10);
            }
            // 是否正确
            if (!$obj->CheckCorrect($params['verify'])) {
                return DataReturn(lang('验证码错误'), -11);
            }
        }

        // LDAP认证
        $samAccount = '';
        if (self::LdapAuthentication($params, $samAccount) === false) {
            return DataReturn(lang('匡威账号或密码错误，注册失败'), -3);
        }

        // 使用AD中的samaccountname
        if (!empty($samAccount)) {
            $data['nickname'] = $samAccount;
        }
        // 数据添加
        ...
        if($user_ret['code'] == 0)
        {
            ...
            {
                return DataReturn(lang('注册成功'), 0, $user_ret);
            }
            return DataReturn(lang('注册成功，请到登录页面登录帐号'));
        } else {
            return $user_ret;
        }
        return DataReturn(lang('注册失败'), -100);
    }
