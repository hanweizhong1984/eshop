...
//    return md5($salt.trim($pwd));
    return  hash("sha512",$salt.trim($pwd));
}
...
function ParamsChecked($data, $params)
{
    ...
    foreach ($params as $v)
    {
        ...
        switch($checked_type)
        {
            ...
            // 是否为纯数字
            case 'num' :
                if(!preg_match("/^[0-9]*$/",($data[$v['key_name']])))
                {
                    return $v['error_msg'];
                }
                break;
...
/**
 * [GoodsSpecType 商品规格校验]
 * @datetime 2021-02-04T21:58:54+0800
 * @param    [字符串或者数组] $spec [商品规格]
 * @return   [boolean]        [正确true, 错误false]
 */
function GoodsSpecType($spec)
{
    if(!empty($spec))
    {
        if(!is_array($spec))
        {   //字符串 [{"type":"尺码","value":"XXL/XXXL"}]
            $specJson = json_decode(htmlspecialchars_decode($spec), true);
        } else {
            $specJson = $spec;
        }

        foreach ($specJson as $v) {
            $specType = $v['type'];
            $specValue = $v['value'];

            // 这不是一个纯中文字串  尺码
            if (!preg_match("/^[".chr(0x80)."-".chr(0xff)."]+$/",$specType)) {
                return false;
            }
            // 位数小于等于8位  XXL/XXXL 35.5
            if(mb_strlen($specValue, 'utf-8')>8){
                return false;
            }
        }
    }
