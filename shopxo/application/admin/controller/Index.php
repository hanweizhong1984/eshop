...
class Index extends Common
{
	...
	{
		// 当前时间大于超时时间，直接跳转修改登陆用户密码
		if( time() > session('admin')["out_time"]){
			return $this->error('密码'.config('shopxo.out_day').'天已到期，请输入新的登录密码', MyUrl('admin/saveinfo', ['id'=>session('admin')["id"]]));
		}
		// 系统信息
